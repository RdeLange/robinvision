...
//RDL TRAIN START
    $('#toolbar a[href="kcact:train"]').click(function() {
        _.train();
        return false;
    });
...
_.train = function() {
    ...
    var dlg = _.dialog(_.label("Training Faces"), html, {width: 301});
    ...
    setTimeout(function() {
        $.ajax({
            ...
            success: function(data) {
                ...
                var span = $('#checkver');
                span.removeClass('loading');
                span.html(_.label("All Faces are trained!"));
            },
            error: function() {
               if (!dlg.html().length)
                    return;
               $('#checkver').removeClass('loading').html(_.label("Something went wrong"));
            }
            });
        }, 1000);
};
